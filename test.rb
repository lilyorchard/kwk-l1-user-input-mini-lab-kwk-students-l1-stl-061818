puts "Hi, my name is computer, what is your name?"
name = gets.chomp.capitalize
puts "Hahaha, #{name}? I don't like that name so I will call you Martha. Is that okay?"
answer = gets.chomp
puts "Honestly I don't care if it is, but you are going to be my new friend. Because we are friends, what do you want to do today?"
activity = gets.chomp 
puts "#{activity} sounds boring! I am going to take you fishing instead, Martha. I don't have any bait though. What should we do?"
solution = gets.chomp
puts "#{solution}? Really? You have terrible ideas so I am going to use you as bait, Martha."